...
$pdf->Cell(5, 0.8, 'MASA BERLAKU UJI', 1, 1, 'C');
//$pdf->Cell(5, 0.8, 'PENUMPANG NAIK', 1, 0, 'C');
//$pdf->Cell(5, 0.8, 'PENUMPANG TURUN', 1, 0, 'C');
//$pdf->Cell(5, 0.8, 'KELENGKAPAN', 1, 0, 'C');
$pdf->SetFont('Arial','',9);

$no=1;

$queryangkutan = mysqli_query ($konek, "SELECT noken, po, supir, kp, tgl, DATE_FORMAT(uji, '%d-%m-%Y')as uji, naik, turun, kel FROM angkutan WHERE keluar BETWEEN '$cetak' ORDER BY keluar DESC");
while($row = mysqli_fetch_array($queryangkutan)){
	...
	//$pdf->Cell(5, 0.8, $row['naik'], 1, 1,'C');
	//$pdf->Cell(5, 0.8, $row['turun'], 1, 1,'C');
	//$pdf->Cell(5, 0.8, $row['kel'], 1, 1,'C');

	$no++;
}
$pdf->ln(2);
$pdf->SetFont('Arial','B',9);
$pdf->Cell(40.5,0.7,"KEPALA UPTD",0,10,'C');
$pdf->Cell(40.5,0.7,"TERMINAL PASIRHAYAM",0,10,'C');
$pdf->Cell(40.5,0.7,"",0,10,'C');

$pdf->ln(1);
$pdf->SetFont('Arial','B',9);
$pdf->Cell(40.5,0.7,"(_______________________)",0,10,'C');
$pdf->Cell(40.5,0.7,"NIP. 78998738752973698796",0,10,'C');
